...
from .prediction import MLStripper, strip_tags, preprocess, vectorize, model_predict, model_predict_single, model_predict_df, pd
...
def load_into_df():
    ...
    df = pd.read_csv('hackernews_comments.csv')
...
def insert_comment_with_model(comment_tuple):
    ...
    try:
        ...
        author = comment_tuple[1]
        text = comment_tuple[5]
        toxicity = round(float(model_predict_single(comment_tuple[5])), 2)
        db_comment=Comment(comment_id=comment_id, author=author, text=text, toxicity=toxicity)
        DB.session.add(db_comment)
